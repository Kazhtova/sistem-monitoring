    ...
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <h2 class="font-black text-2xl text-gray-900 tracking-tight">
                {{ $teknisi->exists ? 'Edit Data Teknisi' : 'Tambah Teknisi Baru' }}
            </h2>
            
            <a href="{{ route('teknisi.master-teknisi.index') }}" 
    ...
    <div class="py-8 min-h-screen bg-slate-50/50">
        <div class="max-w-2xl mx-auto px-4">
            <div class="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
                ...
                <form action="{{ $teknisi->exists ? route('teknisi.master-teknisi.update', $teknisi->id_teknisi) : route('teknisi.master-teknisi.store') }}" method="POST" class="space-y-6">
                    @csrf
                    ...
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2">Nama Lengkap Teknisi</label>
                        <input type="text" name="nama_teknisi" value="{{ old('nama_teknisi', $teknisi->nama_teknisi) }}" required 
                    ...
                    <div class="pt-4 flex justify-end gap-3">
                        <a href="{{ route('teknisi.master-teknisi.index') }}" class="px-5 py-2.5 bg-slate-100 text-slate-600 font-bold rounded-xl hover:bg-slate-200 transition-colors">Batal</a>
